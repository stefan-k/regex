...
#[derive(Debug)]
enum Transition {
    Char { c: char, out: Rc<Option<Rc<State>>> },
...
impl State {
    pub fn new_char(c: char, out: Rc<Option<Rc<State>>>) -> Self {
        State {
...
struct Frag {
    start: Rc<State>,
    // end: Vec<Option<Rc<State>>>,
}

impl Frag {
    // pub fn new(start: Rc<State>, end: Vec<Option<State>>) -> Self {
        ...
        match x {
            '.' => {
                ...
                e1 = Frag::new(Rc::new(State::new_char(
                    e1.start.get_char(),
                    Rc::new(Some(Rc::clone(&e2.start))),
                )));
                stack.push(Frag::new(Rc::clone(&e1.start)));
            }
            c => {
                let s = State::new_char(c, Rc::new(None));
